...
const collectEmployees = function() {
  ...
  while (true) {
    ...
    if(!confirm("Do you want to add another employee?s")){
      break;
    }
  }
...
const displayAverageSalary = function(employeesArray) {
  ...
  let avageSalary = employeesArray.reduce((acc, cur) => acc + cur.salary, 0) / employeesArray.length;
  console.log(`The average employee salary between our ${employeesArray.length} employee(s) is ${avageSalary.toLocaleString("en-US",{
      style:"currency",
      currency:"USD"
    })}`);
